...
│   ├── feature-context-window.md
│   └── feature-tool-call-visibility.md
├── src/
│   ├── main.rs                 # 入口：事件循环、工具调度、对话渲染
│   ├── agent.rs                # （预留）Agent 核心逻辑
│   ├── context/
│   │   ├── mod.rs             # ContextManager — 上下文生命周期管理
│   │   ├── config.rs          # ContextStrategy 配置类型
│   │   ├── strategy.rs        # 四层压缩策略实现
...
在 `main.rs` 的 `initial_model()` 之后配置：
...
DEEPSEEK_BASE_URL=https://api.deepseek.com/v1
# 模型名在 main.rs 中硬编码（当前默认 deepseek-v4-flash）
```
...
4. 在 `main.rs` 的 `initial_tool_manager()` 中注册
