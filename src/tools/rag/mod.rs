...
pub struct RagIndex {
    ...
    hyde_generator: Option<Arc<tokio::sync::Mutex<HydeAgent>>>,
}
...
impl RagIndex {
    ...
    pub async fn search(
        ...
        if let Some(hyde) = &self.hyde_generator {
            let mut guard = hyde.lock().await;
            match guard.generate(query).await {
                Ok(doc) => {
                    eprintln!("[RagIndex] HyDE generated: {}", doc.chars().take(80).collect::<String>());
                    base_queries.push(doc);
                }
                Err(e) => {
                    eprintln!("[RagIndex] HyDE generation failed: {}, use original query only", e);
                }
            }
        }
            ...
            Some(expander) => {
                ...
                for base in base_queries {
                    match guard.expand(&base).await {
                        ...
                        Err(e) => {
                            eprintln!("[RagIndex] MQE expansion failed: {}, use base query", e);
