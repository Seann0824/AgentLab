...
- 上下文被压缩后，检查注入的任务状态恢复上下文"#,
    ...
    // ⭐ 注册 /tools 命令到命令注册表
    // (此处在 tool_manager 和 command_registry 都可用时调用)
    ...
    loop {
        ...
        // ⭐ 检查上下文是否阻塞（Token >= 95%），如果阻塞则强制压缩再发送
        if ctx.is_blocked() {
            eprintln!(
                "\r\x1b[2K⚠️  上下文使用率 {:.0}%，触发强制压缩...",
                ctx.stats().usage_ratio * 100.0,
            );
            let result = ctx.force_compress();
            eprintln!(
                "\r\x1b[2K✅ 强制压缩完成: {} (tokens: {:.0}%)",
                result.description(),
                ctx.stats().usage_ratio * 100.0,
            );
        } else if ctx.is_critical() {
            // 临界状态，尝试工具调用修剪（轻量压缩）
            let _ = ctx.prune_tool_calls();
        }
