...
- 项目同时包含一个可运行的主程序（`crates/agent-lab-core/src/main.rs`，交互式 RAG Agent）和一个示例（`crates/agent-lab-core/examples/novel_generation.rs`，多 Agent 小说创作）。
...
| 测试 | 位置 | 依赖 | 说明 |
|------|------|------|------|
| `test_new_embedder` | `crates/agent-lab-core/src/tools/memory/storage/embedder/ollama_embedder.rs` | 无 | 仅验证默认值 |
| `test_pg_store_add` | `crates/agent-lab-core/src/tools/memory/storage/pg.rs` | PG + DATABASE_URL | 真实写入/清理 |
